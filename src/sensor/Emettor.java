 ...
 * 
 * @author: Lea D - the constructor Emettor, to initialize variables and
 *          informations to send the datas (adress, port)
 * @param: String hostname: adresse or website to which we send the data int
 *                port: the UDP port we send our data (make sure this port
 *                matches that of the receiver) - the send method to send data
 *                (the message) to an adress and port specified in the
 *                constructor
 * @param: String message: message that will be send - the close method to close
 *                the socket
 */
public class Emettor {
	...
	/**
	 * Method to send the data in our parameter to to computer with the address and
	 * to the port initialized in the constructor
	 * 
	 * @param message: type: String, data send to our receiver (figures like float
	 *                 stringified for example)
	 * @throws IOException
	 */
	...
	/**
	 * Main to send data locally or to another computer (use to test the class
	 * Emettor)
	 * 
	 * @param args
	 * @throws Exception
